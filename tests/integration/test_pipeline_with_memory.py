...
def test_code_generation_pipeline_with_memory():
    ...
    has_memory_retrieval = any(
        "memory" in step_key.lower() or "retrieval" in step_key.lower() for step_key in steps.keys()
    )
    has_memory_writer = any(
        "memory" in step_key.lower() and "writer" in step_key.lower() for step_key in steps.keys()
    )
    ...
    if not has_memory_retrieval:
        ...
        if "steps_log" in run_state:
            ...
            has_memory_retrieval = any(
                "memory_retrieval" in str(step_info).lower() for step_info in steps_log.values()
            )
        else:
    ...
    if not has_memory_writer:
        ...
        if "steps_log" in run_state:
            steps_log = run_state["steps_log"]
            has_memory_writer = any(
                "memory_writer" in str(step_info).lower() for step_info in steps_log.values()
            )
        else:
            # Проверим в любом месте результата
            result_str = str(result)
