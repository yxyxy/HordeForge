...
| `ci_fix_pipeline` | Analyze failed CI and create/enrich incident issue with `agent:opened` label | `pipelines/ci_fix_pipeline.yaml` |
| `issue_scanner_pipeline` | Scan staged issues (`agent:opened/planning/ready/fixed`), run planning when required, dispatch `feature_pipeline`, close validated fixed issues | `pipelines/issue_scanner_pipeline.yaml` |
...
| `ci_monitoring_pipeline` | Monitoring flow for CI state and diagnostics | `pipelines/ci_monitoring_pipeline.yaml` |
...
1. `ci_fix_pipeline` captures CI failure and creates/enriches an `agent:opened` issue.
...
3. `feature_pipeline` implements and validates code.
