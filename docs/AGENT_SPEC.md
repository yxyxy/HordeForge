...
Агенты могут использовать LLM через унифицированный интерфейс:

- `agents/llm_api.py` — основной интерфейс для работы с LLM
- Поддержка 18+ провайдеров: OpenAI, Anthropic, Google, Ollama и др.
- Token Budget System для отслеживания стоимости использования
- Поддержка streaming и non-streaming операций

## 14. Использование памяти агентов

Агенты могут использовать систему памяти для:

- Поиска аналогичных решений в исторических данных
- Использования успешных паттернов
- Избегания повторения прошлых ошибок

Интеграция через:
- `rag/memory_retriever.py` — для поиска в памяти
- `orchestrator/hooks.py` — для автоматического сохранения результатов
- `rag/context_builder.py` — для объединения RAG и memory контекста

## 15. Оптимизация контекста

Агенты поддерживают оптимизацию контекста:
- Compression через `rag/context_compressor.py`
- Deduplication через `rag/deduplicator.py`
- Уменьшение размера запросов к LLM
