...
def _parse_args() -> CleanupConfig:
    parser = argparse.ArgumentParser(
        description="Cleanup artifact records older than retention window."
    )
    parser.add_argument(
        "--storage-dir",
        default=".hordeforge_data",
        help="Storage directory containing artifacts.json.",
    )
    parser.add_argument(
        "--retention-days",
        type=int,
...
def run_cleanup(config: CleanupConfig) -> dict[str, int]:
    store = JsonStore(get_current_log_path(config.storage_dir, "artifacts.json"))
    ...
    items = [ArtifactRecord.from_dict(item) for item in store.read_all()]
...
def main() -> int:
    ...
    print(f"cleanup_artifacts removed={result['removed']} kept={result['kept']}")
    return 0
