...
class QwenCodeHandler(ApiHandler):
    ...
    @staticmethod
    def _mask_secret(value: str, keep_prefix: int = 6, keep_suffix: int = 4) -> str:
    ...
    async def create_message(
        ...
        logger.info(
            "qwen_chat_request model=%s base_url=%s messages=%s tools=%s access_token=%s",
            self.qwen_model_id,
            self._base_url(),
            len(messages),
            len(tools or []),
        ...
        try:
            ...
            logger.exception("qwen_chat_request_failed model=%s error=%s", self.qwen_model_id, e)
